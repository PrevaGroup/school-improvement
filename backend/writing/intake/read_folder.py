"""Read a folder of student work, work out whose each file is, and record what was there.
    ...
    python -m writing.intake.read_folder --folder /tmp/11b-oped --section 11b --task fs10-oped \\
...
creates no artifacts — `artifact` belongs to `scoring`, and `writing/scoring/bind.py` reads these rows to
...
log = logging.getLogger("writing.intake.read_folder")
