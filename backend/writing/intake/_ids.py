"""Time-ordered id minting for intake-owned rows.

A deliberate near-copy of `writing/scoring/_ids.py`, for the same reason it is a near-copy of
`app.traces.uuid7`: that one is serving-owned and this module may not import it. Time-ordered ids
keep manifest and file rows sortable by creation without a second index.
"""
