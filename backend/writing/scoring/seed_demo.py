"""Seed the artifacts for an end-to-end run, and remove them again.
    ...
    python -m writing.registry.seed_demo --prompt-versions "$(python -m writing.scoring.prompts)"   # the rubric
    python -m writing.scoring.seed_demo --text-dir /tmp/sip-demo                            # the papers
    python -m writing.scoring.run_scoring --config-key writing-default                         # score
    python -m writing.scoring.seed_demo --purge && python -m writing.registry.seed_demo --purge     # clean up
...
`writing/registry/seed_demo.py` explains why reference identifiers are meaningless UUIDs: a trait
...
def seed(text_dir: pathlib.Path) -> dict:
    ...
    return {"folder": str(text_dir), "files": sorted(written),
            "next": f"python -m writing.intake.read_folder --folder {text_dir} "
...
def main() -> None:
    ...
    ap.add_argument("--purge", action="store_true",
                    help=f"remove run {RUN_ID} and everything it produced (the rubric is "
                         f"`python -m writing.registry.seed_demo --purge`)")
    args = ap.parse_args()
    ...
    print(f"\nNow: python -m writing.scoring.run_scoring --config-key {CONFIG_KEY}")
