...
from writing.scoring.run_scoring import (AlreadyRunning, ConfigurationError, check_configuration,
...
def test_corpus_text_is_preferred_over_the_file_branch():
    from writing.scoring.run_scoring import read_text
...
def test_the_corpus_join_is_scoped_to_the_corpus_tenant():
    ...
    from writing.scoring.run_scoring import _PENDING
...
def test_scoring_reads_the_corpus_table_rather_than_importing_it():
    ...
    from writing.scoring import run_scoring
...
def fake_engine(monkeypatch):
    def install(**kw):
        eng = _Engine(**kw)
        monkeypatch.setattr("writing.scoring.run_scoring.engine", lambda: eng)
        return eng
    return install
...
def test_each_stage_actually_calls_its_own_model():
    ...
    from writing.scoring.rater import AnthropicRater
...
def test_a_corpus_assignment_is_preferred_over_an_intake_lookup():
    ...
    from writing.scoring.run_scoring import _score_one
