...
```bash
cd backend
python -m writing.registry.seed_demo --prompt-versions "$(python -m writing.scoring.prompts)"  # author + lint
python -m writing.scoring.seed_demo --text-dir /tmp/sip-demo                # 2 synthetic papers, bound
python -m writing.scoring.run_scoring --config-key writing-default --dry-run   # calls the model, writes nothing
python -m writing.scoring.run_scoring --config-key writing-default             # scores, writes, transitions
python -m writing.scoring.compose                                           # packet + draft, -> in_review
python -m writing.scoring.seed_demo --purge && python -m writing.registry.seed_demo --purge
```
