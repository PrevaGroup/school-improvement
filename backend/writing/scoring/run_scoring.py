"""Score every artifact waiting in `bound`, and move it to `scored`.
    ...
    python -m writing.scoring.run_scoring --run-id R --tenant public --config-key writing-default
                                  [--limit N] [--dry-run]
...
log = logging.getLogger("writing.scoring.run_scoring")

# The first look. An escalated criterion is written at pass 2 — see `writing/scoring/escalate.py` for
...
def _score_one(eng, artifact: dict, *, tenant: str, config_key: str, dry_run: bool,
    ...
    # anything ambiguous — see `writing/scoring/fit.py` for why those two errors are not comparable.
