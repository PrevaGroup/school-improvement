"""Corpus papers into artifacts, so they go through the pipeline student work goes through.
    ...
    python -m writing.corpus.draw_anchor --json | python -m writing.scoring.bind_corpus --wave 1
...
log = logging.getLogger("writing.scoring.bind_corpus")
...
def main() -> None:
    ...
    if key not in payload:
        raise SystemExit(
            ...
            f"`python -m writing.corpus.draw_anchor --json`.")
    ids = payload[key]
    if not ids:
