...
# Same namespace shape as `writing/registry/seed_demo.py`: derived ids, so re-seeding is idempotent and a
...
# NO LICENCE, AND NO POINTER TO ONE. There was a `LICENCE_NOTE` here naming a permissive licence,
# which was this project stating somebody else's terms from memory; it was replaced with a
# `TERMS_URL`, and that was the same mistake one step removed. Licensing is not a fact this system holds — not the
# terms, not a link to them. It lives in the contracts system. What belongs here is provenance:
# which instrument, transcribed by whom, on what date.
