"""Promote a scoring configuration — that is, publish a rater.
    ...
    python -m writing.scoring.prompts --method cumulative \\
      | python -m writing.registry.promote_configuration --config-key writing-cumulative \\
...
from `python -m writing.scoring.prompts --method <method>` through a pipe — the same seam as
...
def main() -> None:
    ...
    if sys.stdin.isatty():
        raise SystemExit(
            ...
            f"  python -m writing.scoring.prompts --method {a.level_method} "
            f"| python -m writing.registry.promote_configuration ...")
