"""Author the fixture rubric, lint it, and publish it only if the linter allows.
    ...
    python -m writing.registry.seed_demo --prompt-versions "$(python -m writing.scoring.prompts)"
    python -m writing.registry.seed_demo --acknowledge RULE SUBJECTS REASON BY
    python -m writing.registry.seed_demo --purge

THIS FILE EXISTS BECAUSE THE LINTER DID NOT. `writing/registry/lint.py` is a dozen rules, fully tested, and
...
def main() -> None:
    ...
    ap.add_argument("--prompt-versions",
                    help="the pipeline fingerprint, as JSON. Get it with "
                         "`python -m writing.scoring.prompts`.")
    ...
    if not args.prompt_versions:
        ap.error("--prompt-versions is required: the configuration records the prompt fingerprint "
                 "of the pipeline it was promoted against. `python -m writing.scoring.prompts` prints it.")
